...
test('Compare XML responses from two environments', async ({ playwright }) => {
  ...
  async function findDifferences(expected: any, actual: any, path = ''): Promise<string[]> {
    const chalk = (await import('chalk')).default;
    let differences: string[] = [];
    for (let key in expected) {
      const currentPath = path ? `${path}.${key}` : key;
      if (typeof expected[key] === 'object' && expected[key] !== null && actual[key] !== null) {
        differences = differences.concat(await findDifferences(expected[key], actual[key], currentPath));
      } else if (expected[key] !== actual[key]) {
        differences.push(`Path: ${currentPath} - ${chalk.green('Expected:')} ${chalk.green(expected[key])}, ${chalk.red('but got:')} ${chalk.red(actual[key])}`);
      }
    }
    return differences;
  }

  const differences = await findDifferences(jsonResponse1, jsonResponse2);
